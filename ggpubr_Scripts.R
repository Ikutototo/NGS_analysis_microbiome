...
## 多様性指数の統計処理
stat.test_dps <- compare_means(Shannon ~ dps,
    data = alpha_df,
    method = "wilcox.test", label = "p.format"
)
...
ggplot(alpha_df, aes(x = dps, y = Shannon)) +
    ...
    ggsave(
        filename = "Richness_Shannon_dps.png", plot = last_plot(),
...
alpha_df |>
    ggboxplot(
        x = "Fungicide.use", y = "Shannon", color = "Fungicide.use",
        palette = c("#00AFBB", "#E7B800", "#FC4E07"),
        add = "jitter"
    ) +
    geom_pwc(method = "wilcox_test", label = "{p.format}{p.signif}") +
...
stat.test <- compare_means(Shannon ~ Fungicide.use,
    data = alpha_df,
    method = "wilcox.test", label = "p.format"
)
...
ggplot(alpha_df, aes(x = Fungicide.use, y = Shannon)) +
    ...
    stat_pvalue_manual(stat.test,
        label = "【Wilcox.Test】 {p.format}{p.signif}",
...
ggsave(
    filename = "Richness_Shannon_Fungicide.use.png", plot = last_plot(),
    width = 2800, height = 2520, dpi = 300, units = "px",
    path = "~/Documents/RStudio/Novogene/250503/NGS_analysis_microbiome/png"
)
